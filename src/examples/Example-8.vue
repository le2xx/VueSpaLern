<template lang="html">
  <div>
    <h2 class="h2">Урок 8. CRUD</h2>
      ...
      <button class="btn" v-on:click="sortAuto('year', 'up')">Сортировка по возрастанию года</button>
      <button class="btn" v-on:click="sortAuto('year', 'dowm')">Сортировка по убыванию года</button>
      <button class="btn" v-on:click="sortAuto('price', 'up')">Сортировка по возрастанию цены</button>
      <button class="btn" v-on:click="sortAuto('price', 'dowm')">Сортировка по убыванию цены</button>
      ...
      <div>
        Добавляем машины
        <input class="input" v-model="auto.name" placeholder="Название">
        <input class="input" v-model="auto.model" placeholder="Марка">
...
export default {
  ...
  methods: {
    sortAuto: function (n, m = 'up') {
      if (m === 'up') {
        return this.todos.sort((a, b) => a[n] - b[n])
      }
      return this.todos.sort((a, b) => b[n] - a[n])
    },
    filterAuto: function () {
